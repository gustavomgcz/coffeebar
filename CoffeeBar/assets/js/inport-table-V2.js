...
const cabecalho = ["User ID","Nome","E-mail","Celular","Pontos", "Ações"]
buscaDigitado.value = ''
let idUsuarioClicado = ''
let pontosUsuarioClicado = ''

const modal = document.querySelector('.modal')
const inputNomeUsuario = document.querySelector('#nome-usuario')
const inputCelUsuario = document.querySelector('#celular-usuario')
const inputEmailUsuario = document.querySelector('#email-usuario')
const inputPontosUsuario = document.querySelector('#pontos-usuario')
inputNomeUsuario.value = ''
inputCelUsuario.value = ''
inputEmailUsuario.value = ''
inputPontosUsuario.value = ''

const radioAdRem = document.querySelector('.radio-ad')
let radioSelecionado = document.querySelector('input[name="add-rem"]:checked').value

const botaoCancela = document.querySelector('.botao-cancela')
...
function mostraDados(resultadoCorpo, itensPorPagina, paginaAtual) {
    ...
    for (let i = inicioIndex; i < fimIndex && i < resultadoCorpo.length; i++) {
        ...
        botaoVantagens.innerHTML = '<svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg"> <path d="M3.75 4.5a.75.75 0 000 1.5h3a.75.75 0 000-1.5h-3zM10.25 7.5a.75.75 0 01.75.75V9.5h1.25a.75.75 0 010 1.5H11v1.25a.75.75 0 01-1.5 0V11H8.25a.75.75 0 010-1.5H9.5V8.25a.75.75 0 01.75-.75z"></path> <path fill-rule="evenodd" d="M5.25 0a5.25 5.25 0 015.244 5.006 5.25 5.25 0 11-5.488 5.488A5.25 5.25 0 015.25 0zM1.5 5.25a3.75 3.75 0 017.499-.1A5.258 5.258 0 005.15 8.999 3.75 3.75 0 011.5 5.25zm8.75 1.25a3.75 3.75 0 100 7.5 3.75 3.75 0 000-7.5z" clip-rule="evenodd"></path></svg>'
        ...
        botaoVantagens.addEventListener('click', (e) => {
            modal.style.display = 'block'

            idUsuarioClicado = arrayAtualizado[i].id
            pontosUsuarioClicado = arrayAtualizado[i].Pontos
            
            inputNomeUsuario.value = arrayAtualizado[i].Nome
            inputCelUsuario.value = arrayAtualizado[i].Celular
            inputEmailUsuario.value = arrayAtualizado[i].Email
            inputPontosUsuario.value = arrayAtualizado[i].Pontos
...
radioAdRem.addEventListener('click', function(e) {
    if (e.target.type === "radio" && e.target.name === "add-rem"){
        radioSelecionado = e.target.value
    }
})

botaoConfirma.addEventListener('click', (e) => {
    const valorDigitado = Number(document.querySelector('#qtd-pontos').value)
    if (radioSelecionado === "add") {
        pontosUsuarioClicado = pontosUsuarioClicado + valorDigitado
        atualizaPontos(pontosUsuarioClicado)
    } else if (radioSelecionado === "rem") {
        pontosUsuarioClicado = pontosUsuarioClicado - valorDigitado
        atualizaPontos(pontosUsuarioClicado)
    } else {
...
async function atualizaPontos(pontosAtualizados) {
    const urlAtt = `http://localhost:3000/corpo/${idUsuarioClicado}`
    const dados = { Pontos: pontosAtualizados}

    try {
        const resp = await fetch(urlAtt, {
            method: 'PATCH',
            headers: {
